<?php
    ...
    $num = $_GET["num"];
    if ($num == 1) {
        $act_password = mysqli_query($con, "SELECT password FROM Owner_info WHERE Contact_no_1='" . $contact_no . "'");    
    } else {
        $act_password = mysqli_query($con, "SELECT password FROM Tenant_info WHERE Contact_no='" . $contact_no . "'");    
    }
